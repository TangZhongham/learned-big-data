...
module Shell
  ...
  class Shell
    ...
    def hyperbase_table(name)
      hbase.hyperbase_table(name, self)
    end

    def hbase_local_index_table(name)
      hbase.local_index_table(name, self)
    end
    ...
    def hyperbase_admin
      @hyperbase_admin ||= hbase.hyperbase_admin(formatter)
    end

    def index_rebuilder
      @index_rebuilder ||= hbase.index_rebuilder(formatter)
    end
    ...
    def descriptor_admin
      @descriptor_admin ||= hbase.descriptor(formatter)
    end
...
Shell.load_command_group(
  ...
  :commands => %w[
    ...
    rebuild_index
    rebuild_global_index_with_range
    rebuild_global_index_with_timerange
    delete_local_index
    rebuild_local_index
    rebuild_local_index_with_range
    rebuild_local_index_with_timerange
    delete_fulltext_index
    rebuild_fulltext_index
    rebuild_fulltext_index_with_range
    rebuild_fulltext_index_with_timerange
    split_fulltext_index_partition
    locate_region
    client_insert
  ],
  :aliases => {
