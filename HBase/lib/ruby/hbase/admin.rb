...
module Hbase
  class Admin
    ...
    #----------------------------------------------------------------------------------------------
    # query or switch the value of logSwitch  in  HRegionServer
    def htrace(qed , servers)
      result = nil
      if(qed == 'status')
        result = @admin.getHTraceStatus(servers)
      elsif(qed == 'enable')
        result=@admin.enableHTrace(servers)
      elsif(qed == 'disable')
        result=@admin.disableHTrace(servers)
      end
      result.each do | state |
        @formatter.row([ state ])
      end
      puts()
      puts("=>" + java.util.Arrays.toString(result))
    end
    ...
    def hcd(arg, htd)
      ...
      family.setLocalIndex(org.apache.hadoop.hbase.secondaryindex.indexbuilder::SecondaryIndexUtil.convertPatternToBytes(arg.delete(org.apache.hadoop.hbase.HConstants::HYPERBASE_LOCAL_INDEX))) if arg.include?(org.apache.hadoop.hbase.HConstants::HYPERBASE_LOCAL_INDEX)
