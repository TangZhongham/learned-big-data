...
module Hbase
  class Hbase
    ...
    def local_index_table(table, shell)
      ::Hbase::LocalIndexTable.new(configuration, table, shell)
    end
    ...
    def hyperbase_admin(formatter)
      ::Hbase::HyperbaseAdmin.new(configuration, formatter)
    end
    def index_rebuilder(formatter)
      ::Hbase::IndexRebuilder.new(configuration, formatter)
    end
    def descriptor(formatter)
      ::Hbase::DescriptorAdmin.new(configuration,formatter)
    end
    def hyperbase_table(table, shell)
      ::Hbase::HyperbaseTable.new(configuration, table, shell)
    end
  end
end
